...
- No horizontal dividers between sections, and no dividers inside cards. (Thin `borderTop` rules exist only in the **three** modal/footer spots: the GlowPT modal footer, the e-book modal footer, and the page footer.) **The two products on the Available card are separated by spacing, not a rule** — 12px under the heading, 10px title→teaser, and a 28px break between the two products; see Available.
...
- **The newsletter card copy is: header "Newsletter", subhead "The Operator", description "For those who run a physical therapy practice. Delivered every other Tuesday, it takes a clear look at what's working in your practice, what isn't, and what to do about it."** (V36 flipped the header/subhead order.) "The Operator" is the newsletter's name; the old "Notes from the ops chair" standfirst is retired from the masthead (see Source-of-Truth Documents).
