# FranklinAI — Project Instructions (Version 45, Current)

> **Version 45 updates:** **Process change only. No copy, no code, no locked value moved.** **Claude Code now owns the maintenance of this document.** The rule that Code must never edit `CLAUDE.md` is **retired**, and with it the loop where every push came back to claude.ai for a documentation pass and then went out again as a file David placed by hand in two homes. **The new loop:** decisions are argued in claude.ai as before, a handoff note carries both the strings *and the editorial reasoning* into Code, Code pushes the code and then regenerates `CLAUDE.md` whole as a separate second step, and David pastes the finished file back into the claude.ai project instructions. **One placement instead of two**, and the copy that went stale in V44 is no longer maintained by hand.
>
> **Four conditions make this safe, and they are not optional.** (1) **Regenerate whole, never patch** — this is now Code's rule to keep, and it is the load-bearing one; the seventh-versus-eighth bullet error found in V44 survived seven versions precisely because accumulated targeted edits never force a full read. (2) **The doc is written last**, after the code push lands, never mid-task — a file Code is reading as spec must not be a file Code is rewriting, or it drifts to match whatever just happened instead of holding the line against it. (3) **The handoff note must carry the reasoning, not just find/replace strings** — the *why* behind a decision is the one input Code cannot derive from a diff, and without it this document degrades into a changelog. (4) **The repo copy is now authoritative**, and the claude.ai project instructions are the copy that can lag; when the two disagree, the repo wins. See Working in Claude Code vs claude.ai and Process.
>
> **The Content Calendar does not move.** It is not in the repo, Code has no reason to open it, and it stays a claude.ai document. Newsletter drafting, strategy, positioning, and copy decisions all stay in claude.ai too. What moved is the *recording*, not the *deciding*.
...
- **Content Calendar** — the source of truth for all content and marketing. These instructions intentionally contain **no** content/marketing specifics; they live in that file, and it says so itself in its own header. The calendar is **newsletter-led**: the engine is *The Operator*, every other Tuesday, 400–600 words, free, written Sunday on a phone. **Instagram is the only discovery surface** (LinkedIn and Facebook groups were deliberately cut — a founder who does not live on a surface cannot sustain it). The newsletter does depth. **The discovery call is the only door.** The addressee is the **owner**; the office manager is the multiplier who forwards it. The spearhead is **model confusion** ("You're running an insurance practice with cash-model marketing" — corrected in V41; the old "insurance clinic / cash-clinic" phrasing recorded here was stale, and accuracy beat the old clinic/clinic symmetry), across **six spines**: (A) model confusion, (B) plan-of-care completion, (C) team, hiring, retention, and modeling, (D) your numbers, (E) AI in your practice, (F) systems and delegation. The four-video YouTube arc is **parked whole**, not deleted.
  ...
  > - **"cash practice" and "advanced care" are interchangeable, and neither is retired** (corrected in V41; the V37 note claiming one replaced the other was wrong). Use whichever fits the sentence. The site's e-book teaser says "cash practice," the modal says "advanced care," and both are correct.
...
| Footer small print | Inter | 400 | 13px, #8892a4, both lines centered (no per-line nudge) |
...
> The footer brand-row `translateX(-14px)` lockup dial is horizontal only and does not affect this vertical F-vs-crossbar math (see Horizontal Centering).
...
**Text column** (right, flex column, centered, `marginTop: '13.16px'`, **no `transform`**):
1. FranklinAI — Playfair 700, 32px, blue #60a5fa with "AI" in white #ffffff (in a home link). **No per-line nudge.**
2. Solutions, LLC — **Inter 800**, **20px**, **blue #60a5fa**, marginTop 2px (in the home link), **no `transform`**
3. franklinaisolutions.com — Inter 400, 13px, #8892a4, marginTop 8px, **no `transform`**
4. © {year} — Inter 400, 13px, #8892a4, **no `transform`**
...
- **Description** (Inter 15px, #8892a4, marginTop 0, marginBottom 18px, lineHeight 1.5): **"For physical therapy practices. Delivered every other Tuesday, it takes a clear look at what's working and not working in your practice, and what to do about it."** *(V41 — the opening sentence changed from "For those who run a physical therapy practice." to match the Instagram graphic and the issue-top promise. **The identical line also lives in `NewsletterModal`; both move together.**)*
...
- **`NewsletterModal`** (rendered via `{newsletterOpen && <NewsletterModal … />}`, `NewsletterStrip` holds the `newsletterOpen` state): title "The Operator", "Free to subscribe." subline, **the same description line as the card** (V41 wording), then a `borderTop` divider wrapping the email `input` + a ghost "Subscribe for Free" button (both pinned to 44px, flush). See Newsletter Card and Locked Inline Styles.
...
**GlowPT Modal:** Scroll-locked. Title "GlowPT" (marginTop 6px); subline "Clinic subscriptions available." (Inter 14px 600, #8899b0); description tying between-visit engagement to completed plans of care; **eight** feature lines (blue checks, cream text); price $350 / month (Playfair 27.7px, blue); CTA "Book a Free Discovery Call" (`padding: '10px 20px', fontSize: '13px'` — the smallest primary button, **title case, no period**).
...
- **The newsletter card copy is: header "Newsletter", subhead "The Operator", description "For physical therapy practices. Delivered every other Tuesday, it takes a clear look at what's working and not working in your practice, and what to do about it."** *(V41 changed the opening sentence.)* "The Operator" is the newsletter's name; the old "Notes from the ops chair" standfirst is retired from the masthead. **The card's only action is a "Subscribe for Free" button that opens the `NewsletterModal`**; the modal repeats the same description and adds the subline **"Free to subscribe."** above the email field.
...
| **Newsletter card description `<p>`** | fontFamily: Inter, fontSize: '15px', color: '#8892a4', marginTop: '0', marginBottom: '18px', lineHeight: '1.5'. **Text (V41): "For physical therapy practices. Delivered every other Tuesday, it takes a clear look at what's working and not working in your practice, and what to do about it."** Moves with the modal copy below. |
...
| **`NewsletterModal` description `<p>`** | className `modal__text`. **Same V41 text as the card description — the two are duplicated by design and must always move together.** |
...
| **Footer text column `<div>`** | display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '13.16px' (**no `transform`**) |
...
| Footer address `<span>` | fontFamily: Inter, fontSize: '13px', color: '#8892a4', lineHeight: 1.5, marginTop: '8px' (**no `transform`**) |
| **Footer copyright `<span>`** | fontFamily: Inter, fontSize: '13px', color: '#8892a4', lineHeight: 1.5 (text **"© {year}"**; **no `transform`**) |
...
2. **Make the code change** and verify it by string checks — old strings at zero, new strings present, no em dashes, no forbidden words. A predicted line count is not a verification (V43).
3. **Push the code.**
...
- **V45** — **Process change: Claude Code now maintains this document. No copy, no code, and no locked value moved.** The rule banning Code from editing `CLAUDE.md` is retired, along with the loop it forced: every push came back to claude.ai for a documentation pass, went out as a downloaded file, and had to be placed by hand in **two** homes — the repo and the claude.ai project instructions. **V44 is the case study for why that failed.** Four Code sessions pushed ahead of the doc, and the resulting handoff note asked for two edits V41 and V42 had already recorded, which is the signature of a Code session reading a repo `CLAUDE.md` several versions stale. The hand-placement step was the leak. **The new loop:** decisions are argued in claude.ai, a handoff note carries the strings *and the reasoning* into Code, Code pushes the code and then regenerates `CLAUDE.md` whole as a separate second step and pushes it, and David pastes the finished file into claude.ai. One placement instead of two, and the copy that went stale is now written by the tool that lives in the repo. **Four conditions were made explicit because the arrangement is only as good as they are:** (1) **regenerate whole, never patch** — the load-bearing rule, and the V44 bullet-numbering error is the evidence, having survived seven versions because no pass in that stretch forced a full read; (2) **the doc is written after the code push, never mid-task**, which preserves the real reason the old ban existed (the spec being read must not be the file being rewritten); (3) **the handoff note must carry editorial reasoning**, since the *why* is the one input Code cannot derive from a diff and its absence would reduce this file to a changelog; (4) **the repo copy is now authoritative and the claude.ai copy is the one that can lag** — an inversion of every prior version — with a version-number mismatch as the tell in either direction. **What did not move:** the Content Calendar stays a claude.ai document (it is not in the repo and Code has no reason to open it), and strategy, positioning, copy decisions, and newsletter drafting all stay in claude.ai. What moved is the recording, not the deciding. Sections updated: title, header summary (rewritten for V45, V44 folded into "everything else from Version 44," V43 and V42 and V41 and V40 kept as their own carried-forward paragraphs), Working in Claude Code vs claude.ai (tool-split table, handoff-note precedence, the new two-homes note), Source File Rule (which copy goes stale, now inverted), Process (In Claude Code rewritten with the five-step sequence and three rules; In claude.ai scoped; Handoff notes into Code given the reasoning requirement), Change History.
- **V44** — **Documentation only; no code written this pass.** Four Claude Code sessions (`b810743`, `a7ae6ba`, `badc1f0`, `52204a6`) pushed copy edits straight to `main` ahead of the doc; this version catches the doc up, written after reading the live `App.jsx` directly rather than from the handoff note alone. **Three live copy changes recorded:** **(1) The newsletter description's tail changed in both places it appears** — "it takes a clear look at what's working in your practice, what isn't, and what to do about it" became **"it takes a clear look at what's working and not working in your practice, and what to do about it."** The V41 audience opener "For physical therapy practices." is unchanged. Five quotations of the full line in this document were updated together, which is the *copy that appears twice must move twice* rule applied to the documentation as well as the code. **(2) The e-book teaser** opens **"A downloadable guide"** instead of "A short guide," stating the delivery format at the point of interest and echoing the modal subline's "yours to download." **(3) The GlowPT teaser** became **"An app for physical therapy patients to stay engaged between visits, with zero work for your therapists."** Two shifts: the subject moved from the practice to the patient, and "keep patients engaged" became "stay engaged," putting the agency on the patient. The **Positioning note was reworded** as a result: it had listed the GlowPT teaser as one of three places the *practice* is named, which is no longer literally true. The words "physical therapy" still qualify a scanning visitor and the closing clause still addresses the owner, so the niche-forward front door holds; only the teaser's subject moved. **Two corrections found by reading the file rather than trusting the note:** **(4) the GlowPT modal's EMR bullet sits seventh, not eighth.** V37 introduced it as "the eighth," meaning it brought the count to eight, and every version since has repeated that as a position. The placement V37 described ("right after 'Zero work for your therapists'") was correct all along. **(5) The real eighth bullet had never been recorded in this file** — *"A free weekly roster summary and a clinic dashboard to follow activity, trends, and flags."* All eight are now written out in live order under GlowPT Modal so a bullet can be cited by number without guessing. **One new open item:** the word **"flags"** in bullet 8 may be stale, since GlowPT may no longer use flags. It is **not** changed this pass — GlowPT is still being built and lives in its own repo — and the task is recorded as: verify against the finished app before pilot, then reconcile the modal copy. **One new note in the Source File Rule:** the repo's `CLAUDE.md` can go stale independently of the claude.ai project instructions, and the tell is a handoff note asking for a change this file already documents, which is exactly what happened here (the note listed the e-book modal paragraphs and the About bio as outstanding when V41 and V42 had already recorded them). Sections updated: title, header summary (rewritten for V44, V43 folded into "everything else from Version 43," V42 and V41 and V40 kept as their own carried-forward paragraphs), Source File Rule (snapshot in sync + the stale-`CLAUDE.md` note), Business (Positioning note), Newsletter Card, Brand Kit typography, Page Sections (Newsletter + Available, both teasers, GlowPT Modal feature list), Copy Rules, Locked Inline Styles (newsletter description rows), Change History. No locked style value, price, or structural rule moved.
- **V43** — **Documentation only; no code change.** Two corrections and one new standing rule, all fallout from the V42 pass. **(1) The V42 line-count claim is corrected.** V42 recorded "Net −1 line (637 → 636 by editor count)" for the About bio rewrite. That figure was a prediction written into the handoff note *before* the push and then copied into this file as though measured. It was wrong: the old first paragraph and its replacement both wrap to three lines, so `App.jsx` was **636 lines before the push and 636 after**. The V42 entry now carries an inline correction; the bio copy itself is untouched and correct. **(2) The Source File Rule note is updated — the snapshot is back in sync.** During V42 the project `App.jsx` was one push behind and still carried the V41 bio, which is the documented hand-replacement lag and the normal state immediately after a push. David has since replaced it; it now reads 636 lines with the V42 bio and passes every string check. **(3) New Process rule: a predicted line count is an estimate, not a verification, and never a fact for this file.** Handoff notes may carry an expected delta as a rough signal, but string checks (old at zero, new present, no em dashes, no forbidden words) are the real proof, and any line count recorded here must be measured on the pushed file rather than carried over from the note. This is precisely the failure that produced correction (1), and it is worth a rule because this document's entire value is that the facts in it hold up. Sections updated: title, header summary (rewritten for V43, V42 folded into "everything else from Version 42," V41 and V40 kept as their own carried-forward paragraphs), Source File Rule, Process (Handoff notes into Code), Change History (V42 entry corrected inline, V43 entry added). No section of the site spec, no locked value, and no copy string moved.
- **V42** — **Two copy edits pushed to `App.jsx`, both in the `About` component; the founder bio rewritten.** One Claude Code session, run from a claude.ai handoff note; no styles, structure, or other section moved. **(1) Bio paragraph 1 replaced whole.** It opened "David is the operations manager for a busy clinical practice. He's worked in both nonprofit and for-profit businesses, building systems and processes to keep everything running smoothly." Three problems, all real: a present-tense job title ties FranklinAI's identity to an employer, it goes stale silently whenever the job changes (David's hours at that practice were cut to part-time), and it reduces a decades-long operations career to one room, which is the opposite of the edge the Content Calendar has described all along ("The edge is not 'David works in a physical therapy clinic.' It is thirty years of operations"). It now reads, in one sentence: **"David has spent decades building systems and processes in both nonprofit and for-profit businesses, across healthcare, research, and education."** **(2) Bio paragraph 2's opening phrase changed from "That same drive" to "That experience."** Required, not cosmetic — "that same drive" referred to "keep everything running smoothly," which edit (1) removed, so it would have pointed at nothing. The rest of paragraph 2 is untouched and still closes on "what a business needs." Verified zero occurrences of "operations manager," "busy clinical practice," and "That same drive," one each of the new strings, no em dashes, no forbidden words. *(V43 correction: this entry originally read "Net −1 line (637 → 636 by editor count)." That was a prediction copied from the handoff note, never measured. Both the old and the new first paragraph wrap to three lines, so the file was **636 lines before the push and 636 after** — no net change. See the V43 Process rule on predicted line counts.)* **The bio now names no job title, no employer, and no niche.** All three are deliberate: the page qualifies a visitor three times (headline, newsletter card, GlowPT teaser) long before the bio, so the bio is free to be the broad one, which is the job this file has assigned it since V23. New standing rules: a **Copy Rule banning any job title, employer, or present-tense employment fact from all site copy**, and a four-point guard in **Bio (About Section)** covering the missing job title, the missing niche, the two-axes clause structure (sector vs. tax status, never compressed into one list), and the paragraph-2 dependency on paragraph 1. **Note on the snapshot:** when this version was written the project `App.jsx` was one push behind and still carried the V41 bio — the documented hand-replacement lag, recorded in the Source File Rule. Sections updated: title, header summary (rewritten for V42, V41 folded into "everything else from Version 41," V40 kept as its own carried-forward paragraph), Source File Rule (snapshot lag), Business (Positioning note + the "where the niche is named" note), Page Sections (About), Copy Rules, Bio (About Section), Change History.
- **V41** — **Three copy edits pushed to `App.jsx`, plus four documentation corrections.** One Claude Code session, run from a claude.ai handoff note; no styles, structure, or prices moved. **(1) E-book modal body paragraphs 1 and 2 rewritten.** The old text carried **two** retired framings at once: the front-desk-versus-therapist split (paragraph 1's opening and paragraph 2's closing) and "Nobody decided to run both" (paragraph 2's opening). Both are banned by the Content Calendar's craft rules — the split is factually wrong about how a practice runs, and the accident framing lets the owner off as someone this happened to. New paragraph 1 opens "Your whole team is being asked to sell advanced care and get patients to the end of their plan of care at the same time," lifted from Issue 1's finished hook so the shelf and the newsletter say the same thing; new paragraph 2 opens "You didn't set out to run two businesses, but you chose every piece that built the second one" and lands on "So the team gets mixed signals about what the practice is driving at, and every patient feels the seams." Paragraph 3, the subline, the `$37` price line, and the ghost Buy button are untouched. **(2) The newsletter audience line changed from "For those who run a physical therapy practice." to "For physical therapy practices." in BOTH locations** — the `NewsletterStrip` card description and the `NewsletterModal` description. It now matches the Instagram graphic and the issue-top promise; **David's call is one line on every surface**, closing the prior pass's open question about whether the promise moves with the card (it does). **(3) Net +2 lines** (634 → 636); verified zero occurrences of "Nobody decided," "front desk," and the old audience line, two occurrences of the new one, no em dashes, no forbidden words. **The four documentation corrections:** **Issue 1's ship date is September 1, 2026** (this file said August 4; the calendar had moved it to August 18 before it slipped again — a new Process rule now points dates at the calendar rather than restating them here); **the spearhead's wording corrected** to "insurance practice with cash-model marketing"; **the e-book's status restated honestly** (card and modal live, book unwritten, checkout unwired, both due before September 1 — V40's "Live" overstated it and contradicted the calendar); and **the V37 claim that "cash practice" replaced "advanced care" corrected** — both are interchangeable and neither is retired. New standing rules added: a *"copy that appears twice must move twice"* Design Rule with a grep check, a Copy Rule banning both retired framings from all site copy, a note that a handoff note supersedes this file for its session and Code must not patch `CLAUDE.md`, and a Process section on what a good handoff note contains. Sections updated: title, header summary (rewritten for V41, V40 folded into "everything else from Version 40"), Source File Rule (snapshot in sync), Working in Claude Code vs claude.ai (wrong-repo risk + handoff-note precedence), Source-of-Truth Documents (ship date, spearhead wording, the two retired framings, the terminology correction, the promise line), Business (e-book status), The Offer Ladder (table status + sequence date + open items), Pricing Model, Newsletter Card (description + the duplicated-line warning), Brand Kit typography (newsletter description row) and colors (the #070f24-reads-black note), Design Rules, Page Sections (Newsletter + Available/`EbookModal` body), Copy Rules, Forbidden Words (added "upsell" and "treatment room"), Locked Inline Styles (both newsletter description rows + e-book modal body row), CSS/JSX Boundary, Process, Change History.
- **V40** — **Locked in the newsletter form's wiring + success behavior (documentation only).** One Claude Code session, **no code change**. Records how the `NewsletterModal` form will be wired to Kit when David hands over the form ID: (1) **JS-handled submit, not a plain form POST** — a plain POST to Kit's endpoint navigates the browser away and would destroy the modal; a `fetch` (or Kit's v3 API / React embed) keeps the subscriber in the modal. Field named `email_address`. (2) **On success, confirm in the modal then auto-close** (~2s), X available sooner. (3) **Confirmation copy tracks Kit's double opt-in setting** — "Almost there. Check your email to confirm." if on (Kit default), "You're in." if off. (4) **On error, keep the modal open** with a gentle inline message; the button takes a brief "Subscribing…" disabled state. Sections updated: title, header summary, Tech Stack, Page Sections, Change History. The form remains unwired.
- **V39** — **Newsletter signup moved into a modal; reverses the V28 "keep inline" decision.** (1) The email field left the card; the card now rests as header → subhead → description → a single **"Subscribe for Free"** ghost button. (2) **New `NewsletterModal`** — the site's **fourth** modal, mirroring `EbookModal`/`GlowPTModal`: title "The Operator", subline **"Free to subscribe."**, the same description line, then a `borderTop` divider (the site's fourth) wrapping the email input + Subscribe button. `NewsletterStrip` gained a `newsletterOpen` state. (3) **Rationale:** a bold Subscribe button is the visual magnet, so many visitors reach for the button first — with an inline field that was a dead click, not a saved tap. Accepted tradeoff: one extra tap for a field-first user. Still unwired.
- **V38** — **Available card spacing tightened by eye on the live site.** Heading→first-product 20px → **12px**; between-products 36px → **28px**. The 10px title→teaser and 16px teaser→button gaps unchanged. New rhythm: heading →12→ title →10→ teaser →16→ button →28→ title →10→ teaser →16→ button. Supersedes V34's values.
- **V37** — **Both product-card teasers rewritten for parallel structure; GlowPT's EMR claim moved into its modal.** (1) E-book teaser → "A short guide for owners running two business models in one building, an insurance practice and a cash practice, and making them work together." (2) GlowPT teaser shortened to one sentence. (3) "Runs alongside any EMR. Nothing to set up, nothing to integrate." preserved as a new **eighth** `GlowPTModal` feature bullet. *(V41 note: this entry's claim that "cash practice" **replaced** "advanced care" was wrong — the two are interchangeable and both stand. The teaser wording itself is unchanged.)*
- **V36** — **Newsletter masthead flipped + redundant teaser sentence cut.** (1) The lockup reversed and de-italicized: **"Newsletter"** became the blue upright header at the standard section tier, **"The Operator"** the white upright subhead beneath. **The V26 masthead exception is retired** — nothing on the page is italic or off-tier. (2) E-book teaser trimmed. (3) The italic Playfair `1,800` face is now unused but retained in `index.html`.
...
- **V33** — **Available card spacing refined for cross-card consistency.** Heading→first-product pulled back from 32px to the standard ~10px; both product titles gained `marginBottom: '10px'`. Superseded by V34/V38.
- **V32** — **Reconciliation e-book shipped to the Available card as a second, self-serve product, listed first, above GlowPT; two documented decisions reversed.** New `EbookModal` mirroring `GlowPTModal` (title, "A self-serve guide. $37, yours to download." subline, three body paragraphs, $37 price, ghost "Buy the Guide" at `href="#"`). The Available card went from one product to two. **Two reversals:** e-book **first** (was "below GlowPT"), and a **modal** (was "links out to an external checkout"). Third divider added. *(V41 note: paragraphs 1 and 2 of that modal body were rewritten in V41; and this entry's "the e-book is live before Issue 1 (Aug 4, 2026)" is doubly stale — the book was never written and the date moved twice, to August 18 and then to September 1.)*
- **V31** — **Fine-print nudges retired for true centering; two dead files deleted.** All four per-line `translateX(-2px)` nudges removed (measured center offset −2px → 0 on every line). `src/components/Nav.jsx` and `Footer.jsx` plus their orphaned CSS deleted; `src/components/` now holds only `KiteLogo.jsx`. Footer "Solutions, LLC" `marginTop` doc-drift fixed to match the code (2px).
...
- **V29** — **Wordmark-subline font: Playfair Display 400 → Inter 700, nav and footer.** A deliberate mixed-type lockup — sans-serif bold subline under the serif "FranklinAI" wordmark.
...
- **V27** — **Wordmark lockup: "Philadelphia" → "Solutions, LLC", enlarged sublines, trimmed copyright, and a footer recenter.** The wordmark's second line became the legal suffix in blue; sublines enlarged; the footer copyright trimmed to "© {year}"; the footer text-column inner pull removed so the footer is built exactly like the nav.
- **V26** — **Operator-card rebalance, italic masthead, tighter card typography, and two live-code drift fixes.** First session run in Claude Code (the instructions now live in the repo as `CLAUDE.md`). Card-heading `lineHeight: '1.2'` added (the real fix for the loose header gap, which had been inheriting the body's 1.6); card padding tightened; the `index.html` font import corrected to include italic; the meta description cleaned of "software" and an em dash. *(The italic masthead itself was retired in V36.)*
- **V25** — **Newsletter card masthead restructure, Subscribe-button alignment fix, and terminology correction.** "Strip" retired for "card"; the Subscribe button pinned to the input's 44px with `marginTop: '0'` cancelling the `.btn` class margin; the promise-line coupling retired.
