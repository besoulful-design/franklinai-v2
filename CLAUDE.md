# FranklinAI — Project Instructions (Version 31, Current)

> **Version 31 updates:** One Claude Code working session. The per-line `translateX(-2px)` fine-print nudges were **removed** so the wordmark sublines and footer small print sit truly centered under "FranklinAI"; two dead component files were deleted. No copy, pricing, or layout changes.
>
> **The `translateX(-2px)` "fine-print nudge" is retired from all four lines it was on.** David flagged that "Solutions, LLC" looked visibly off-center under "FranklinAI" in both the nav and footer. Diagnosis: the column centering was correct (nav `align-self: center`, footer `alignItems: center`), but the leftover `translateX(-2px)` — originally tuned for the old "Philadelphia" line — shoved each subline 2px left, turning a symmetric ~8px/8px inset into a lopsided ~6px/10px. Removing the transform lands every line dead-centered (measured center offset −2px → **0**). Cleared from: the **nav** "Solutions, LLC" span, the **footer** "Solutions, LLC" span, the **footer** "franklinaisolutions.com" line, and the **footer** "© {year}" line. **The Fine-Print Left-Nudges table is now empty — do not re-add a per-line `translateX` to these lines. If a subline ever looks off-center again, the fix is the column centering, not a nudge.**
>
> **Two dead component files were deleted:** `src/components/Nav.jsx` and `src/components/Footer.jsx`, plus their orphaned `Nav.css` and `Footer.css`. Nothing imported them — the live nav and footer are defined inline inside `App.jsx`, and these stale files still read "Philadelphia" with old class-based styling. They were a trap for editing the wrong file. **`src/components/` now holds only `KiteLogo.jsx` (the live logo mark).** The nav/footer wordmark lives in `App.jsx`, full stop.
>
> **Everything else from Version 30 remains in force:** the "Solutions, LLC" subline in **Inter 800** (nav + footer synced; V29 introduced Inter, V30 took it to 800), blue #60a5fa (nav `clamp(25.6px, 2.72vw, 31.5px)`, footer `20px`) — now with **no** per-line nudge (V31); the nav lockup dial `translateX(-26px)` with its wordmark dropped so the F sits **level with the kite crossbar** (`marginTop: clamp(13.98px, calc(42.25px - 2.44vw), 19.28px)`); the footer lockup dial `translateX(-14px)`; the newsletter email field **kept inline** (a modal was considered and declined); the wordmark lockup carrying the full legal name — nav and footer read **"FranklinAI" / "Solutions, LLC"**; the footer copyright trimmed to **"© {year}"**; the footer built like the nav (kite + wordmark, one 4px flex gap, no inner text-column pull); and, carried forward from Version 26, the Operator-card rebalance ("The Operator" title `clamp(34px, 5vw, 50px)` in true italic, "Newsletter" label `clamp(30px, 5vw, 35px)`); card-heading `lineHeight: '1.2'` (About keeps 1.1); tightened card padding (`paddingTop: '16px'`, `paddingBottom: '24px'`, sides 40); the "Subscribe for Free" ghost-button label; the corrected `index.html` font import (`ital,wght@0,700;0,800;1,800`) and cleaned `<meta name="description">`; the newsletter card (title/label/description lockup) with "The Operator" kept as the title and the "name never travels alone" standfirst retired; the Subscribe/input 44px alignment fix and its `marginTop: '0'` override; the niche-forward front door (headline "Built for Your Practice", subline "Built to Handle HIPAA" as moat/proof); all buttons title case with no period; the newsletter platform **Kit**, with Kit Commerce the front-runner for the e-book checkout; the legal entity **FranklinAI Solutions LLC** with "FranklinAI" as the fictitious name; the Content Calendar as the single canonical content/marketing document; the contact email `david@franklinaisolutions.com`; the reconciliation e-book as a planned second Available shelf (~$37, self-serve, external checkout, not built, ships after issue 1); the named offer-ladder rungs and their prices; the Business Plan deleted; the Claude Code vs claude.ai scoping; the Source File Rule; the stack boundary (site + McKenzie → Supabase; GlowPT → AWS); the F-vs-crossbar vertical-alignment method; Clarity retired from the site; gold logo-bolt-only; "in plain English" and "software" banned site-wide; no em dashes in site copy; the $350 GlowPT price; the GlowPT exercise-plan exclusion.
