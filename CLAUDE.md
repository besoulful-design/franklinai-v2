# FranklinAI — Project Instructions (Version 32, Current)

> **Version 32 updates:** One Claude Code working session. The **reconciliation e-book went live on the Available card as a second, self-serve product, listed first, above GlowPT.** New `EbookModal` component, new product row, tuned two-product card spacing. Two previously documented decisions were **reversed** this pass (see below and Change History). GlowPT is unchanged in every respect and simply moves to second position.
>
> **The e-book is now built and live — it is no longer "planned."** It is the first product on the Available card: product name **"What Your Practice Actually Sells"** (white sub-title, per the gold-is-logo-only rule), a short teaser, and a **More Info** ghost button that opens the new **`EbookModal`** (mirrors `GlowPTModal`: scroll-lock, X close, backdrop close). The modal carries a positioning subline, three body paragraphs, the **$37** price (Playfair, blue #60a5fa, one-time, no "/ month" suffix), and a **"Buy the Guide"** ghost button. **The Buy button is a dead link (`href="#"`) for now** — the real external checkout URL (Kit Commerce front-runner) gets dropped in later. GlowPT keeps its title, teaser, More Info button, full modal, and $350, and moves to second.
>
> **Two documented decisions were reversed this pass:**
> 1. **The e-book is listed FIRST, above GlowPT** (the instructions had said GlowPT first, e-book "below GlowPT"). Rationale: the e-book answers the exact problem the newsletter diagnoses (Spine A, model confusion), so it belongs directly under the newsletter as one continuous thought; GlowPT (Spine B, plan-of-care completion) is a separate problem. Cheaper-first also fits the page's free-to-$10k climb.
> 2. **The e-book uses a modal, not a link out to an external checkout** (the instructions had said its button "links out to an external checkout, not to a modal"). Rationale: consistency with GlowPT's More Info → modal → CTA pattern, selling the book on-brand before the reader leaves the site, and turning the off-site jump into a deliberate exit. The purchase link now lives **inside** the modal.
>
> **The e-book modal footer adds the site's THIRD divider.** To mirror GlowPT's modal, the e-book modal's price/button block uses the same hairline `borderTop` (`rgba(96, 165, 250, 0.12)`). David approved this — the two product modals now read as one pattern. **There are now three intentional dividers on the site: the GlowPT modal footer, the e-book modal footer, and the page footer.** Everywhere the docs used to say "only two dividers," it is now three.
>
> **Two-product card spacing (tuned live to David's eye):** heading → first product **32px** (the e-book `<h3>` carries `marginTop: '32px'`, which collapses with the `card-heading` marginBottom); **16px** within each product (title → text → button, with the e-book `card-text` pinned to `marginBottom: '0'` to match GlowPT's last-of-type); **48px** between the two products (the GlowPT `<h3>` carries `marginTop: '48px'`). No horizontal rule between the products — separation is spacing only, per the divider-question resolution. The result reads as two tight product clusters under one heading.
>
> **Everything else from Version 31 remains in force:** the V31 removal of all four per-line `translateX(-2px)` fine-print nudges (nav + footer sublines and footer small print centered via column centering alone) and the deletion of the two dead component files (`src/components/` holds only `KiteLogo.jsx`); the "Solutions, LLC" subline in **Inter 800** (nav + footer synced; V29 introduced Inter, V30 took it to 800), blue #60a5fa (nav `clamp(25.6px, 2.72vw, 31.5px)`, footer `20px`) — with **no** per-line nudge; the nav lockup dial `translateX(-26px)` with its wordmark dropped so the F sits **level with the kite crossbar** (`marginTop: clamp(13.98px, calc(42.25px - 2.44vw), 19.28px)`); the footer lockup dial `translateX(-14px)`; the newsletter email field **kept inline** (a modal was considered and declined); the wordmark lockup carrying the full legal name — nav and footer read **"FranklinAI" / "Solutions, LLC"**; the footer copyright trimmed to **"© {year}"**; the footer built like the nav (kite + wordmark, one 4px flex gap, no inner text-column pull); and, carried forward from Version 26, the Operator-card rebalance ("The Operator" title `clamp(34px, 5vw, 50px)` in true italic, "Newsletter" label `clamp(30px, 5vw, 35px)`); card-heading `lineHeight: '1.2'` (About keeps 1.1); tightened card padding (`paddingTop: '16px'`, `paddingBottom: '24px'`, sides 40); the "Subscribe for Free" ghost-button label; the corrected `index.html` font import (`ital,wght@0,700;0,800;1,800`) and cleaned `<meta name="description">`; the newsletter card (title/label/description lockup) with "The Operator" kept as the title and the "name never travels alone" standfirst retired; the Subscribe/input 44px alignment fix and its `marginTop: '0'` override; the niche-forward front door (headline "Built for Your Practice", subline "Built to Handle HIPAA" as moat/proof); all buttons title case with no period; the newsletter platform **Kit**, with Kit Commerce the front-runner for the e-book checkout; the legal entity **FranklinAI Solutions LLC** with "FranklinAI" as the fictitious name; the Content Calendar as the single canonical content/marketing document; the contact email `david@franklinaisolutions.com`; the reconciliation e-book now **live** as the first product on the Available card ($37, self-serve, opens `EbookModal` with an in-modal Buy button whose real checkout URL is still pending); the named offer-ladder rungs and their prices; the Business Plan deleted; the Claude Code vs claude.ai scoping; the Source File Rule; the stack boundary (site + McKenzie → Supabase; GlowPT → AWS); the F-vs-crossbar vertical-alignment method; Clarity retired from the site; gold logo-bolt-only; "in plain English" and "software" banned site-wide; no em dashes in site copy; the $350 GlowPT price; the GlowPT exercise-plan exclusion.
...
- No horizontal dividers between sections, and no dividers inside cards. (Thin `borderTop` rules exist only in the **three** modal/footer spots: the GlowPT modal footer, the e-book modal footer, and the page footer.) **The two products on the Available card are separated by spacing, not a rule** — 32px under the heading, 48px between products; see Available.
...
**Two-product vertical spacing (locked, tuned live in V32):**
- **Heading → first product: ~32px.** The e-book `<h3>` carries `marginTop: '32px'`, which collapses with the card-heading's marginBottom to a measured 32px gap. (Do not read this as 42px — adjacent block margins collapse to the larger of the two.)
- **Within each product: 16px** from teaser to button. The e-book's teaser carries inline `marginBottom: '0'` so it matches GlowPT's teaser (which is `.card-text:last-of-type`, margin-bottom 0 from CSS); both buttons then sit 16px below via their own `marginTop: '16px'`.
- **Between the two products: 48px.** The GlowPT `<h3>` carries `marginTop: '48px'`.
- The rhythm — 32px under the heading, 16px inside each product, 48px between products — makes the two products read as tight clusters under one heading, without a rule. This was arrived at after a flush (0px) first attempt read as one merged product.

**Product 1 — the e-book (listed FIRST):**
- **Title** (Playfair 800, clamp(30px, 5vw, 35px), **white #ffffff**, lineHeight 1.2, **marginTop 32px**): **"What Your Practice Actually Sells"**. White per the sub-title rule (no gold), same treatment as GlowPT's name.
- **Teaser** (`card-text`, inline **`marginBottom: '0'`**): **"A short, downloadable guide for owners running one clinic that bills insurance and sells advanced care without the two halves fighting. Name what your practice actually sells, so your whole team can say it."**
...
- **Title** (Playfair 800, clamp(30px, 5vw, 35px), **white #ffffff**, lineHeight 1.2, **marginTop 48px**): "GlowPT"
...
| **E-book product name `<h3>`** (V32; listed first) | fontFamily: Playfair Display, fontWeight: '800', fontSize: 'clamp(30px, 5vw, 35px)', color: '#ffffff', lineHeight: '1.2', **marginTop: '32px'** (text "What Your Practice Actually Sells"; the marginTop collapses with the heading's marginBottom to a measured 32px heading→product gap) |
...
| GlowPT product name `<h3>` | fontFamily: Playfair Display, fontWeight: '800', fontSize: 'clamp(30px, 5vw, 35px)', color: '#ffffff', lineHeight: '1.2', **marginTop: '48px'** (V32 — the between-products gap; was no marginTop when GlowPT was the only product) |
