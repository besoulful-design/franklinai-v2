# FranklinAI — Project Instructions (Version 28, Current)

> **Version 28 updates:** One Claude Code working session of nav and footer **lockup micro-adjustments** plus a newsletter button relabel. Three positioning dials moved and one label changed; no copy-strategy, pricing, or structural changes. The F-vs-crossbar and `translateX`-dial *methodology* is unchanged; only locked values moved.
>
> **The nav lockup was nudged right and its wordmark dropped to sit level with the kite crossbar.** Two nav-only moves: (1) the nav brand lockup dial moved **`translateX(-26px)` → `translateX(-22px)`** (a 4px nudge right); (2) the nav wordmark `marginTop` clamp was raised 3px at every stop — **`clamp(10.98px, calc(39.25px - 2.44vw), 16.28px)` → `clamp(13.98px, calc(42.25px - 2.44vw), 19.28px)`** — so the capital "F" optical center now sits **level with the kite crossbar** instead of 3px above it. The alignment *methodology* is unchanged; only the target moved from `(crossbar − 3px)` to `crossbar`. **The footer was not touched by these two moves.** See Vertical Alignment, Horizontal Centering, Nav, and Locked Inline Styles.
>
> **The footer lockup was nudged a hair left.** The footer brand-row dial moved **`translateX(-12px)` → `translateX(-14px)`** — a 2px "smidge," deliberately smaller than the nav's 4px nudge. This is the footer's single lockup dial; nothing else in the footer changed, and the nav was not touched. See Horizontal Centering and Footer.
>
> **The nav and footer dials still differ on purpose — now by 8px, not 14px.** After this pass the nav is `-22px` and the footer is `-14px`. The asymmetry rationale is unchanged: the nav's kite is *relatively smaller* next to its larger wordmark (kite-to-text ratio ~0.22 vs the footer's ~0.27), so the nav leans further left to *look* equally centered. Both numbers were hand-tuned on the live site. **Do not "reconcile" the two dials to one value** — the visual result, not the pixel number, is what matches. See Horizontal Centering.
>
> **The newsletter Subscribe button was relabeled "Subscribe for Free."** The ghost-button label changed from **"Subscribe Free" → "Subscribe for Free"** — the verb-led phrasing reads better on a button than the terser V26 wording, and still states the price at the point of action. Title case, no period, ghost style, everything else unchanged. See Newsletter Card, Copy Rules, Typography, and Locked Inline Styles.
>
> **A modal for the email field was considered and declined this pass.** Moving the newsletter email input into a modal opened by the Subscribe button (to declutter the card) was weighed and **rejected**: the newsletter is the cold rung / "small yes," and putting a tap in front of the field would cost signups on the site's lowest-intent action. **The inline email field stays.** Do not re-propose the modal unless David asks. See Newsletter Card.
>
> **Everything else from Version 27 remains in force:** the wordmark lockup carrying the full legal name — nav and footer read **"FranklinAI" / "Solutions, LLC"**, the "Solutions, LLC" subline in blue #60a5fa (nav `clamp(25.6px, 2.72vw, 31.5px)`, footer `20px`, each with the per-line `translateX(-2px)` fine-print nudge); the footer copyright trimmed to **"© {year}"**; the footer built like the nav (kite + wordmark, one 4px flex gap, no inner text-column pull); and, carried forward from Version 26, the Operator-card rebalance ("The Operator" title `clamp(34px, 5vw, 50px)` in true italic, "Newsletter" label `clamp(30px, 5vw, 35px)`); card-heading `lineHeight: '1.2'` (About keeps 1.1); tightened card padding (`paddingTop: '16px'`, `paddingBottom: '24px'`, sides 40); the "Subscribe for Free" ghost-button label; the corrected `index.html` font import (`ital,wght@0,700;0,800;1,800`) and cleaned `<meta name="description">`; the newsletter card (title/label/description lockup) with "The Operator" kept as the title and the "name never travels alone" standfirst retired; the Subscribe/input 44px alignment fix and its `marginTop: '0'` override; the niche-forward front door (headline "Built for Your Practice", subline "Built to Handle HIPAA" as moat/proof); all buttons title case with no period; the newsletter platform **Kit**, with Kit Commerce the front-runner for the e-book checkout; the legal entity **FranklinAI Solutions LLC** with "FranklinAI" as the fictitious name; the Content Calendar as the single canonical content/marketing document; the contact email `david@franklinaisolutions.com`; the reconciliation e-book as a planned second Available shelf (~$37, self-serve, external checkout, not built, ships after issue 1); the named offer-ladder rungs and their prices; the Business Plan deleted; the Claude Code vs claude.ai scoping; the Source File Rule; the stack boundary (site + McKenzie → Supabase; GlowPT → AWS); the F-vs-crossbar vertical-alignment method; Clarity retired from the site; gold logo-bolt-only; "in plain English" and "software" banned site-wide; no em dashes in site copy; the $350 GlowPT price; the GlowPT exercise-plan exclusion.
...
| Ghost buttons ("More Info", "Case Study", "Subscribe for Free") | Inter (btn--ghost) | 600–700 | 15px, **title case, no period** |
...
> The `translateX(-2px)` nudge on "Solutions, LLC" (nav and footer) is horizontal only and does not affect this vertical F-vs-crossbar math. Neither does the footer brand-row `translateX(-14px)` lockup dial (see Horizontal Centering). (V27 removed the footer text column's former `translateX(-22px)` inner snug-pull entirely.)
...
- **Nav brand `<a>`: `transform: 'translateX(-22px)'`** — the lockup-level dial (moves kite + wordmark together). *(V28: nudged right from `translateX(-26px)` → `translateX(-22px)` at David's request; the nav was looking good and this was a small refinement, footer untouched.)* It is deliberately *more* negative than the footer's because the nav's kite is smaller relative to its wordmark; the extra left lean is what makes the nav *look* as centered as the footer (see the asymmetry note below).
...
> **Why the nav and footer dials differ, and why that is correct (values current as of V28).** Both wordmarks are built the same way (one 4px kite gap). But the nav's lockup dial is `-22px` while the footer's is `-14px`. That asymmetry is intentional: the nav's kite is *relatively smaller* next to its larger wordmark (kite-to-text width ratio ~0.22 vs the footer's ~0.27), so the heavier nav wordmark pulls the optical center rightward more. The extra `-8px` of lean cancels that, landing the nav's *wordmark* at roughly the same small offset-from-center as the footer's. **Do not "reconcile" the two dials to one number** — the visual result, not the pixel value, is what matches. *(V28 nudged the nav right `-26→-22` and the footer left `-12→-14`, narrowing the gap between the two dials from 14px to 8px; both were hand-tuned live and remain deliberately different.)*
...
- `padding: '6px 24px'`
- Brand link inline: `gap: '4px'`, `alignItems: 'flex-start'`, `transform: 'translateX(-22px)'` (V28; was `translateX(-26px)`)
...
| Nav brand `<a>` | gap: '4px', alignItems: 'flex-start', transform: 'translateX(-22px)' (V28; was `translateX(-26px)`) |
