...
- No horizontal dividers between sections, and no dividers inside cards. (Thin `borderTop` rules exist only in the **three** modal/footer spots: the GlowPT modal footer, the e-book modal footer, and the page footer.) **The two products on the Available card are separated by spacing, not a rule** — standard 10px gaps (heading→title and title→teaser, consistent with the other cards) plus a 48px break between the two products; see Available.
...
**Two-product vertical spacing (locked; tuned live, final values V33):**
- **Heading → first product: ~10px.** The e-book `<h3>` carries **no marginTop** — the gap is just the `card-heading`'s marginBottom, exactly like every other card (measured 10px, identical to Custom Work). *(V33 pulled this back from a briefly-tried 32px; consistency with the other cards won.)*
- **Title → teaser: 10px** on each product. Both product `<h3>` titles carry inline **`marginBottom: '10px'`** (matching `.card-title`'s 10px used site-wide). *(V33 added this; before, the titles sat flush on their teasers at 0px.)*
...
- **Between the two products: 48px.** The GlowPT `<h3>` carries `marginTop: '48px'` (in addition to its `marginBottom: '10px'`).
- The rhythm — heading →10→ title →10→ teaser →16→ button →48→ title →10→ teaser →16→ button — keeps the heading consistent with the other cards while a clear 48px break separates the two products, without a rule. (History: a flush 0px first attempt read as one merged product; a 32px heading push over-corrected and broke card consistency; the final answer is standard 10px gaps plus the 48px between-products break.)

**Product 1 — the e-book (listed FIRST):**
- **Title** (Playfair 800, clamp(30px, 5vw, 35px), **white #ffffff**, lineHeight 1.2, **marginBottom 10px**, no marginTop): **"What Your Practice Actually Sells"**. White per the sub-title rule (no gold), same treatment as GlowPT's name.
...
- **Title** (Playfair 800, clamp(30px, 5vw, 35px), **white #ffffff**, lineHeight 1.2, **marginTop 48px**, **marginBottom 10px**): "GlowPT"
...
| **E-book product name `<h3>`** (V32; listed first) | fontFamily: Playfair Display, fontWeight: '800', fontSize: 'clamp(30px, 5vw, 35px)', color: '#ffffff', lineHeight: '1.2', **marginBottom: '10px'** (text "What Your Practice Actually Sells"; **no marginTop** — heading→product gap is the standard `card-heading` 10px, V33; the 10px marginBottom gives title→teaser breathing room) |
...
| GlowPT product name `<h3>` | fontFamily: Playfair Display, fontWeight: '800', fontSize: 'clamp(30px, 5vw, 35px)', color: '#ffffff', lineHeight: '1.2', **marginTop: '48px'** (V32 — the between-products gap; was no marginTop when GlowPT was the only product), **marginBottom: '10px'** (V33 — title→teaser breathing room) |
