# FranklinAI — Project Instructions (Version 40, Current)

> **Version 40 updates:** One Claude Code working session — **documentation only, nothing on the live site changed.** Locked in the newsletter form's intended **wiring and success behavior** for when David creates the Kit form and hands over the form ID (per David). The plan: a **JS-handled submit** (a `fetch`, not a plain form POST — a plain POST navigates the browser to Kit's response page and would destroy the modal); on success, **an in-modal confirmation message, then auto-close** the modal after a short delay; **confirmation copy that tracks Kit's double opt-in setting** — "Almost there. Check your email to confirm." if double opt-in is on (Kit's default), a plain "You're in." if David turns it off; and a **keep-the-modal-open error state** with a brief "Subscribing…" disabled state on the button to prevent double sends. The field stays named `email_address`. The full plan lives in **Tech Stack**. The form remains **unwired** until the form ID arrives; this version only records how it will be built.
>
> **Everything else from Version 39 remains in force:** the V39 newsletter signup in `NewsletterModal` (the card rests as header → subhead → description → a single "Subscribe for Free" button that opens the modal; the modal carries the "Free to subscribe." note, the description, and the email field); the V38 Available-card spacing (heading→product 12px, a 28px break between the two products); the V37 parallel product teasers (e-book "A short guide for owners running two business models in one building…", GlowPT "An app for physical therapy practices to keep patients engaged between visits, with zero work for your therapists.") and the V37 eighth `GlowPTModal` bullet "Runs alongside any EMR. Nothing to set up, nothing to integrate."; the V36 newsletter-masthead flip ("Newsletter" the blue upright header, "The Operator" the white upright subhead, the masthead/italic exception retired); and, carried forward, the V35 hero-label tightening (**"Custom Apps & Tools"** label `lineHeight` **1.1** and `marginBottom` **`clamp(2px, 1vw, 8px)`**); the reconciliation e-book **live** as the first product on the Available card, above GlowPT ($37, self-serve, product name "What Your Practice Actually Sells", teaser "A short guide for owners running two business models in one building…" — V37, a More Info ghost button opening **`EbookModal`** — subline "A self-serve guide. $37, yours to download.", three body paragraphs, $37 price, and a ghost "Buy the Guide" button whose real checkout URL is still pending at `href="#"`); the two **reversed** decisions that put it there — **listed first** (was "below GlowPT") and opening a **modal** (was "links out to an external checkout, not to a modal"), both settled; the two-product Available card spacing (heading→product 12px, title→teaser 10px, teaser→button 16px, and a 28px break between the two products, no rule — V38 values); four intentional `borderTop` dividers (the GlowPT modal footer, the e-book modal footer, the **newsletter modal form divider** added in V39, and the page footer); GlowPT unchanged and now second; the four-modal scroll-lock (`EbookModal`, `GlowPTModal`, `CaseStudyModal`, `NewsletterModal`); the V31 removal of all four per-line `translateX(-2px)` fine-print nudges (nav + footer sublines and footer small print centered via column centering alone) and the deletion of the two dead component files (`src/components/` holds only `KiteLogo.jsx`); the "Solutions, LLC" subline in **Inter 800** (nav + footer synced; V29 introduced Inter, V30 took it to 800), blue #60a5fa (nav `clamp(25.6px, 2.72vw, 31.5px)`, footer `20px`) — with **no** per-line nudge; the nav lockup dial `translateX(-26px)` with its wordmark dropped so the F sits **level with the kite crossbar** (`marginTop: clamp(13.98px, calc(42.25px - 2.44vw), 19.28px)`); the footer lockup dial `translateX(-14px)`; the newsletter email field **now in a modal** opened by the card's "Subscribe for Free" button (V39 — reverses the V28 inline decision); the wordmark lockup carrying the full legal name — nav and footer read **"FranklinAI" / "Solutions, LLC"**; the footer copyright trimmed to **"© {year}"**; the footer built like the nav (kite + wordmark, one 4px flex gap, no inner text-column pull); and the newsletter masthead in its **V36 flipped** form ("Newsletter" the blue `card-heading` header at `clamp(36px, 5.5vw, 58px)` upright, "The Operator" the white sub-title beneath at `clamp(30px, 5vw, 35px)` upright — no italic anywhere, the masthead exception retired); card-heading `lineHeight: '1.2'` (About keeps 1.1); tightened card padding (`paddingTop: '16px'`, `paddingBottom: '24px'`, sides 40); the "Subscribe for Free" ghost-button label; the `index.html` font import (`ital,wght@0,700;0,800;1,800` — the italic `1,800` face now unused after the V36 flip but retained) and cleaned `<meta name="description">`; the newsletter card (header/subhead/description lockup, then a single "Subscribe for Free" button) with **"Newsletter" as the header and "The Operator" as the subhead** (V36 flip) and the "name never travels alone" standfirst retired; the Subscribe/input 44px alignment fix and its `marginTop: '0'` override (now on the modal's form row); the niche-forward front door (headline "Built for Your Practice", subline "Built to Handle HIPAA" as moat/proof); all buttons title case with no period; the newsletter platform **Kit**, with Kit Commerce the front-runner for the e-book checkout; the legal entity **FranklinAI Solutions LLC** with "FranklinAI" as the fictitious name; the Content Calendar as the single canonical content/marketing document; the contact email `david@franklinaisolutions.com`; the named offer-ladder rungs and their prices; the Business Plan deleted; the Claude Code vs claude.ai scoping; the Source File Rule; the stack boundary (site + McKenzie → Supabase; GlowPT → AWS); the F-vs-crossbar vertical-alignment method; Clarity retired from the site; gold logo-bolt-only; "in plain English" and "software" banned site-wide; no em dashes in site copy; the $350 GlowPT price; the GlowPT exercise-plan exclusion.
...
**The masthead is a three-part lockup (component `NewsletterStrip`), flipped in V36 so the label leads and the publication name sits beneath it. Both lines are upright — there is no italic on the card anymore:**

- **Header "Newsletter"** — the blue `card-heading`, at the **standard section-heading tier** so it reads like every other card heading (Available, GlowPT, etc.): `fontSize: 'clamp(36px, 5.5vw, 58px)'`, upright (**no `fontStyle`**), `lineHeight: '1.2'`, `marginBottom: 'clamp(4px, 1.5vw, 10px)'`. It names the section plainly for a cold visitor. **Blue via `card-heading`.** *(V36 promoted "Newsletter" from a white sub-label into the header slot; see Header Sizing.)*
- **Subhead "The Operator"** — a white Playfair sub-title beneath the header, `fontFamily: 'Playfair Display'`, `fontWeight: '800'`, `color: '#ffffff'`, `fontSize: 'clamp(30px, 5vw, 35px)'`, `lineHeight: '1.1'`, `marginTop: '0'`, `marginBottom: '14px'`, **upright (no italic)**. **"The Operator" is the newsletter's title/name** — it names the reader who *operates* the practice, not David (V26 kept the name; the "name never travels alone" standfirst rule stays retired). **White per the sub-title rule (no gold),** at the standard secondary sub-title tier, matching GlowPT and McKenzie — see Header Sizing. *(V36 moved "The Operator" out of the header into this white sub-title slot and dropped its italic.)*
...
| Divider line | rgba(96, 165, 250, 0.12) — used in the GlowPT modal footer, the **e-book modal footer** (V32), the **newsletter modal form divider** (V39), and the page footer (four intentional dividers) |
| Text primary | #f0e6d3 |
| Text muted | #8892a4 |
...
| Newsletter header ("Newsletter") | Playfair Display | 800 | **clamp(36px, 5.5vw, 58px)**, blue #60a5fa via `card-heading`, **upright (no italic)**, lineHeight 1.2, marginBottom clamp(4px, 1.5vw, 10px). The standard section-heading tier — as of V36 "Newsletter" is the card header, sitting in-tier with the other six section headings (see Header Sizing). |
| Newsletter subhead ("The Operator") | Playfair Display | 800 | **clamp(30px, 5vw, 35px)**, **white #ffffff**, **upright (no italic)**, marginTop 0, marginBottom 14px, lineHeight 1.1. The white secondary sub-title beneath the header — as of V36 "The Operator" is the subhead (was the italic blue title in V26–V35). |
| Newsletter description | Inter | 400 | 15px, #8892a4, marginTop 0, marginBottom 18px, lineHeight 1.5 |
| Newsletter email input (in `NewsletterModal`, V39) | Inter | 400 | **16px** (prevents iOS zoom), #f0e6d3 on rgba(255,255,255,0.04), 1px border rgba(96,165,250,0.3), radius 8px, padding 10px 14px, height 44px, minHeight 44px, box-sizing border-box |
| Section card headings (all seven) | Playfair Display | 800 | **clamp(36px, 5.5vw, 58px)**, blue #60a5fa via `card-heading`, **lineHeight 1.2** (About keeps 1.1 for its two lines), **marginBottom clamp(4px, 1.5vw, 10px)** — Newsletter, Available, Custom Work, How It Works, Custom Pricing, Behind FranklinAI, Let's Build. (As of V36 "Newsletter" is a standard in-tier heading; the old smaller-italic "The Operator" masthead exception is retired.) |
...
> **Font-import note (V26 origin, V36 update):** V26 corrected the import to `ital,wght@0,700;0,800;1,800` (the live `index.html` had shipped `Playfair+Display:wght@700;800` with no italic at all) so the then-italic "The Operator" masthead rendered in **true** italic. **As of V36 the masthead is upright and nothing on the page is italic, so the `1,800` italic face is now unused** — it is left in the import for now (harmless; drop it only if David asks). Upright 700/800 remain the faces actually in use.
...
**Section card headings stay blue.** All seven are blue via the `card-heading` class: **"Newsletter,"** "Available," "Let's Build," "Custom Work," "How It Works," "Custom Pricing," and **"Behind FranklinAI"** (whose "AI" is white — see below). (As of V36 "Newsletter" is the newsletter card's blue heading and is upright, in-tier with the rest; "The Operator" is now a white sub-title, not a heading.)
...
There are **three** distinct heading tiers on the page. **As of V36 there is no masthead exception** — every heading sits in its tier, and nothing on the page is italic. Do not collapse the tiers or give individual headings their own one-off sizes.
...
2. **Section card headings** — unified at **clamp(36px, 5.5vw, 58px)**, blue, **lineHeight 1.2** (About keeps 1.1 for its two lines), each with **marginBottom clamp(4px, 1.5vw, 10px)**: **Newsletter**, Available, Custom Work, How It Works, Custom Pricing, Behind FranklinAI, Let's Build. (Behind FranklinAI is the only one set on two lines. **As of V36 "Newsletter" is a full member of this tier** — the old smaller-italic "The Operator" masthead exception is retired.)
3. **Secondary sub-titles** — unified at **clamp(30px, 5vw, 35px)**, **all white**: the newsletter **"The Operator"** subhead, the e-book **"What Your Practice Actually Sells"**, GlowPT, McKenzie Arm Care, step titles, pricing-tier titles, David Peterson.

Price numbers are a separate fixed size: **27.7px** (not a heading tier).

> **The masthead exception is retired (V36).** From V26 through V35, "The Operator" was a smaller, true-italic `card-heading` (`clamp(34px, 5vw, 50px)`, `fontStyle: 'italic'`) sitting *above* a white "Newsletter" sub-label — a sanctioned one-off so the publication name read as a masthead. **V36 reversed the lockup and removed the exception entirely:** **"Newsletter"** is now the blue `card-heading` at the **standard** section-heading tier (`clamp(36px, 5.5vw, 58px)`, upright), and **"The Operator"** is the white secondary sub-title beneath it (`clamp(30px, 5vw, 35px)`, upright), matching GlowPT and McKenzie. **No heading on the page is italic or off-tier anymore.** Do not reintroduce the italic "The Operator" title or the smaller masthead size unless David asks.
...
On the newsletter card specifically, the heading gap feeds the "The Operator" subhead (which then feeds the description), so the shared card-heading gap sits between "Newsletter" and "The Operator" (V36 order).
...
- **No heading on the page is italic** — as of V36 every heading, including the newsletter card's "Newsletter" header, is upright and in-tier. (The old italic "The Operator" masthead was retired in V36; "The Operator" is now a white upright sub-title.) See Header Sizing.
- No horizontal dividers between sections, and no dividers inside cards. (Thin `borderTop` rules exist only in the **four** modal/footer spots: the GlowPT modal footer, the e-book modal footer, the newsletter modal form divider (V39), and the page footer.) **The two products on the Available card are separated by spacing, not a rule** — 12px under the heading, 10px title→teaser, and a 28px break between the two products; see Available.
...
All four modals (`EbookModal`, `GlowPTModal`, `CaseStudyModal`, `NewsletterModal`) lock page scroll while open via a `useEffect` that sets `document.body.style.overflow = 'hidden'` on mount and restores it on unmount. Each closes on X click or backdrop click. (`EbookModal` was added in V32 and `NewsletterModal` in V39; both mirror `GlowPTModal` exactly on all three behaviors.)
...
- **Header** (`card-heading`, clamp(36px, 5.5vw, 58px), blue, **upright**, lineHeight 1.2): **"Newsletter"**, marginBottom clamp(4px, 1.5vw, 10px). The standard section-heading tier, in-tier with the other six headings (V36 flip — see Header Sizing).
- **Subhead** (Playfair 800, clamp(30px, 5vw, 35px), **white #ffffff**, **upright**, marginTop 0, marginBottom 14px, lineHeight 1.1): **"The Operator"**. The newsletter's name/title, now a white sub-title beneath the header (V36 flip; was the italic blue title in V26–V35). White per the sub-title rule (see Header Sizing).
- **Description** (Inter 15px, #8892a4, marginTop 0, marginBottom 18px, lineHeight 1.5): **"For those who run a physical therapy practice. Delivered every other Tuesday, it takes a clear look at what's working in your practice, what isn't, and what to do about it."**
- **Subscribe button** (`btn btn--ghost`, label "Subscribe for Free", marginTop 0): the card's only action, directly under the description. Its `onClick` sets `newsletterOpen` to open the `NewsletterModal`. **No email field on the card** (V39 — the field moved into the modal).
- **`NewsletterModal`** (rendered via `{newsletterOpen && <NewsletterModal … />}`, `NewsletterStrip` holds the `newsletterOpen` state): title "The Operator", "Free to subscribe." subline, the same description line, then a `borderTop` divider wrapping the email `input` + a ghost "Subscribe for Free" button (both pinned to 44px, flush). See Newsletter Card and Locked Inline Styles.
...
- **Heading → first product: 12px.** The e-book `<h3>` carries **`marginTop: '12px'`**, which collapses with the `card-heading`'s marginBottom to a measured 12px gap. *(V38 tightened this from V34's 20px — David wanted a little less air under the heading. History: V33 standard 10px → V34 20px → V38 12px.)*
...
- **Between the two products: 28px.** The GlowPT `<h3>` carries `marginTop: '28px'` (in addition to its `marginBottom: '10px'`). *(V38 tightened this from V34's 36px to bring the GlowPT subhead closer to the e-book's More Info button. History: V32 48px → V34 36px → V38 28px.)*
- The rhythm — heading →12→ title →10→ teaser →16→ button →28→ title →10→ teaser →16→ button — keeps the heading close to its first product while a clear (but tighter) 28px break separates the two products, without a rule. (History: a flush 0px first attempt read as one merged product; a 32px heading push over-corrected; V33 reset to standard 10px / 48px; V34 landed on 20px heading / 36px between-products; V38 tightened both by eye on the live site to 12px / 28px.)
...
- **Teaser** (`card-text`, inline **`marginBottom: '0'`**): **"A short guide for owners running two business models in one building, an insurance practice and a cash practice, and making them work together."** *(V37 rewrite: leads with the modal's "two business models in one building" framing and ends on the positive; "cash practice" is the canonical model-confusion term, replacing "advanced care"; the earlier "without the two halves fighting" wording is retired. Parallels the GlowPT teaser's "An app for …" shape.)*
...
- **Teaser** (`card-text`, no inline overrides — it is `:last-of-type`): **"An app for physical therapy practices to keep patients engaged between visits, with zero work for your therapists."** *(V37 shortened this to one sentence for parallel structure with the e-book teaser; the old second sentence's "Runs alongside any EMR" moved into the `GlowPTModal` as a feature bullet — see GlowPT Modal below.)*
...
**GlowPT Modal:** Scroll-locked. Title "GlowPT" (marginTop 6px); subline "Clinic subscriptions available." (Inter 14px 600, #8899b0); description tying between-visit engagement to completed plans of care; **eight** feature lines (blue checks, cream text) — as of V37 the eighth is **"Runs alongside any EMR. Nothing to set up, nothing to integrate."**, placed right after "Zero work for your therapists. No building, no monitoring, no calls." (it moved here from the old GlowPT teaser's second sentence); price $350 / month (Playfair 27.7px, blue); CTA "Book a Free Discovery Call" (`padding: '10px 20px', fontSize: '13px'` — the smallest primary button, **title case, no period**).
...
- **The newsletter card copy is: header "Newsletter", subhead "The Operator", description "For those who run a physical therapy practice. Delivered every other Tuesday, it takes a clear look at what's working in your practice, what isn't, and what to do about it."** (V36 flipped the header/subhead order.) "The Operator" is the newsletter's name; the old "Notes from the ops chair" standfirst is retired from the masthead (see Source-of-Truth Documents). **The card's only action is a "Subscribe for Free" button that opens the `NewsletterModal`** (V39); the modal repeats the description and adds the subline **"Free to subscribe."** (the free note) above the email field.
...
| **Newsletter header `<h2>`** | className `card-heading`, fontSize: 'clamp(36px, 5.5vw, 58px)', lineHeight: '1.2', marginBottom: 'clamp(4px, 1.5vw, 10px)' (text "Newsletter" — **upright, no `fontStyle`**; the standard section-heading tier, in-tier with the other six. V36 promoted it into the header slot) |
| **Newsletter subhead `<p>`** | fontFamily: Playfair Display, fontSize: 'clamp(30px, 5vw, 35px)', fontWeight: '800', color: '#ffffff', marginTop: '0', marginBottom: '14px', lineHeight: '1.1' (text "The Operator" — **upright, no italic**; the white secondary sub-title beneath the header. V36 moved it here from the old italic blue title slot) |
...
| **Newsletter card Subscribe `<button>`** (V39) | className `btn btn--ghost`, type "button", **onClick opens `NewsletterModal`** (`setNewsletterOpen(true)`), style **marginTop: '0'** (label **"Subscribe for Free"**, no period). The card's only action, directly under the description; no email field on the card. |
| **`NewsletterModal` title `<h2>`** (V39) | className `modal__title`, marginTop: '6px' (text "The Operator") |
| **`NewsletterModal` subline `<p>`** (V39) | fontFamily: Inter, fontSize: '14px', fontWeight: '600', color: '#8899b0', marginBottom: '16px' (text **"Free to subscribe."** — the free note; same slot/style as the GlowPT and e-book modal sublines) |
| **`NewsletterModal` description `<p>`** (V39) | className `modal__text` (same description line as the card) |
| **`NewsletterModal` form divider `<div>`** (V39) | borderTop: '1px solid rgba(96, 165, 250, 0.12)', paddingTop: '24px' (wraps the form row; the site's fourth divider) |
...
| **E-book product name `<h3>`** (V32; listed first) | fontFamily: Playfair Display, fontWeight: '800', fontSize: 'clamp(30px, 5vw, 35px)', color: '#ffffff', lineHeight: '1.2', **marginTop: '12px'** (V38 — 12px heading→product gap, tightened from V34's 20px), **marginBottom: '10px'** (title→teaser breathing room). Text "What Your Practice Actually Sells". |
...
| GlowPT product name `<h3>` | fontFamily: Playfair Display, fontWeight: '800', fontSize: 'clamp(30px, 5vw, 35px)', color: '#ffffff', lineHeight: '1.2', **marginTop: '28px'** (the between-products gap — V32 introduced it at 48px, V34 tightened to 36px, V38 to 28px), **marginBottom: '10px'** (V33 — title→teaser breathing room) |
...
**Heading spacing:** Every `card-heading` sets its own `lineHeight: '1.2'` (About 1.1) and `marginBottom` inline via the shared clamp; no `step-stack` or headshot below a heading carries a negative top margin. As of V36 every heading is upright and in-tier — the newsletter "Newsletter" header uses the standard section-heading `fontSize` and no `fontStyle` (the old smaller-italic "The Operator" treatment is gone).
...
- **V40** — **Locked in the newsletter form's wiring + success behavior (documentation only).** One Claude Code session, **no code change** — nothing on the live site moved. Records how the `NewsletterModal` form will be wired to Kit when David hands over the form ID, so the decision does not get lost: (1) **JS-handled submit, not a plain form POST** — a plain POST to Kit's endpoint navigates the browser away and would destroy the modal; a `fetch` (or Kit's v3 API / React embed) keeps the subscriber in the modal. Field named `email_address`. (2) **On success, confirm in the modal then auto-close** (~2s), X available sooner. (3) **Confirmation copy tracks Kit's double opt-in setting** — "Almost there. Check your email to confirm." if on (Kit default), "You're in." if off; no em dashes/forbidden words. (4) **On error, keep the modal open** with a gentle inline message; the button takes a brief "Subscribing…" disabled state to block double sends. It stays a small self-contained JSX addition (submit handler + success/error state). Sections updated: title, header summary (rewritten for V40, V39 folded into "everything else from Version 39"), Tech Stack (full wiring plan), Page Sections (Newsletter "Not wired" pointer), Change History. The form remains unwired; this only specs the build.
- **V39** — **Newsletter signup moved into a modal; reverses the V28 "keep inline" decision.** One Claude Code session, decided by David against the live site (no copy or pricing change). (1) **The email field left the card.** The newsletter card now rests as header → subhead → description → a single **"Subscribe for Free"** ghost button (inline `marginTop: '0'`); no email input on the card. (2) **New `NewsletterModal`** — the site's **fourth** modal, mirroring `EbookModal`/`GlowPTModal` (scroll-lock via `useEffect`, X close, backdrop close). Contents: title "The Operator" (`modal__title`, marginTop 6px), subline **"Free to subscribe."** (Inter 14px/600/#8899b0 — the free note), the same description line (`modal__text`), then a `borderTop` divider (`rgba(96,165,250,0.12)`, paddingTop 24px — the site's **fourth** divider) wrapping the email input + a "Subscribe for Free" button. `NewsletterStrip` gained a `newsletterOpen` state and the modal render. (3) **Rationale:** a bold Subscribe button is the visual magnet, so many visitors reach for the button first — with an inline field that was a dead/awkward click, not a saved tap. Button-first is now the intended flow; the card is cleaner; the pattern matches the site's existing modals. Accepted tradeoff: one extra tap for a field-first user. The 44px input/button alignment fix moved with the form into the modal (still `marginTop: '0'` + `height`/`minHeight`/`boxSizing`). Still unwired — the Kit endpoint attaches to the modal's form later. Sections updated: title, header summary (rewritten for V39, V38 folded into "everything else from Version 38"; inline-field / three-modal / three-divider standing items updated to modal / four / four), Newsletter Card (form row rewritten as button + `NewsletterModal`; V28 note reversed), Brand Kit colors (divider row 3→4) + typography (email-input row note), Scroll Lock (four modals), Design Rules (four divider spots), Page Sections (Newsletter subsection), Copy Rules (modal subline locked), Locked Inline Styles (card button + full `NewsletterModal` rows), CSS/JSX Boundary (NewsletterStrip note), Tech Stack (Kit form now in modal), Change History. Code pushed to `main` as a separate commit ahead of this doc (Netlify auto-deploy).
- **V38** — **Available card spacing tightened by eye on the live site.** One Claude Code session, two spacing values on the two-product Available card (no copy, pricing, or structural change), tuned against the deployed site. (1) **Heading→first-product tightened from 20px to 12px:** the e-book `<h3>`'s `marginTop` dropped 20→12 (collapses with the card-heading's ~10px marginBottom to a measured 12px), reducing the air under the "Available" heading. (2) **Between-products tightened from 36px to 28px:** the GlowPT `<h3>`'s `marginTop` dropped 36→28, pulling the GlowPT subhead closer to the e-book's More Info button. The 10px title→teaser and 16px teaser→button gaps are unchanged. New rhythm: heading →12→ title →10→ teaser →16→ button →28→ title →10→ teaser →16→ button. Supersedes V34's 20px/36px values. Sections updated: title, header summary (rewritten for V38, V37 folded into "everything else from Version 37"), Design Rules (product-spacing line), Available (spacing block + both product-title specs), CSS/JSX Boundary (Available card note), Locked Inline Styles (e-book + GlowPT product-name rows), Change History. Code pushed to `main` as a separate commit ahead of this doc (Netlify auto-deploy).
- **V37** — **Both product-card teasers rewritten for parallel structure; GlowPT's EMR claim moved into its modal.** One Claude Code session, decided by David against the live site (no pricing or structural change). (1) **E-book teaser rewritten** from "A downloadable guide for owners running one clinic that bills insurance and sells advanced care without the two halves fighting." to **"A short guide for owners running two business models in one building, an insurance practice and a cash practice, and making them work together."** — David found "without the two halves fighting" clunky; the new line borrows the modal's "two business models in one building" framing and ends on the positive, and **"cash practice" replaces "advanced care"** (canonical model-confusion term). "short" returns (V35 had cut it) and "downloadable" drops (the modal still states the format). (2) **GlowPT teaser shortened** to one sentence for a parallel "An app for … with zero work" shape: dropped the second sentence "Runs alongside any EMR, with zero work for your therapists." and folded "zero work" into the first. (3) **"Runs alongside any EMR" preserved as a new eighth `GlowPTModal` feature bullet** — "Runs alongside any EMR. Nothing to set up, nothing to integrate." — inserted right after the "Zero work for your therapists…" bullet (the claim was not in the modal before, so the teaser trim would otherwise have dropped it from the site). Guardrails verified: no em dashes, no forbidden words, "cash practice" framing consistent with the modal and the newsletter spearhead. Sections updated: title, header summary (rewritten for V37, V36 folded into "everything else from Version 36"), Business (e-book purpose line reworded off "two halves fighting"), Available (both teaser specs), Page Sections / GlowPT Modal (feature-line count 7→8 + new bullet), Change History. The e-book and GlowPT modal bodies, prices, and buttons are otherwise unchanged. Code pushed to `main` as a separate commit ahead of this doc (Netlify auto-deploy).
- **V36** — **Newsletter masthead flipped + redundant teaser sentence cut.** One Claude Code session, decided by David against the live site (no pricing or structural change). (1) **Masthead reversed and de-italicized:** the newsletter card lockup went from "The Operator" (blue, italic, `clamp(34px, 5vw, 50px)`, above) / "Newsletter" (white, below) to **"Newsletter" as the blue upright header** at the standard section-heading tier (`clamp(36px, 5.5vw, 58px)`, `lineHeight: '1.2'`, no `fontStyle`) and **"The Operator" as the white upright subhead** beneath (`clamp(30px, 5vw, 35px)`, Playfair 800). **The V26 "sanctioned masthead exception" is retired** — nothing on the page is italic or off-tier anymore. (2) **E-book teaser trimmed:** dropped the trailing sentence "Name what your practice actually sells, so your whole team can say it." (redundant with the `EbookModal`, which already says "say it in one sentence your whole team can repeat"); the teaser now ends at "…without the two halves fighting." (3) **Font-import note:** the italic Playfair `1,800` face is now unused but retained in `index.html` (harmless; drop only if David asks). Sections updated: title, header summary (rewritten for V36, V35 folded into "everything else from Version 35"), Source-of-Truth Documents (Calendar card note), Newsletter Card (masthead lockup rewritten), Brand Kit typography (Newsletter header/subhead rows + section-headings row + font-import note), Logo Accent (color table + all-seven note), Header Sizing (exception retired + tiers 2/3), Card Heading Spacing (gap order + line-height note), Design Rules (no-italic-heading rule), Page Sections (Newsletter subsection), Available (e-book teaser copy), Copy Rules (no-period headings + newsletter card copy), Locked Inline Styles (Newsletter header/subhead rows), CSS/JSX Boundary (heading spacing), Change History. Code pushed to `main` as a separate commit ahead of this doc (Netlify auto-deploy).
